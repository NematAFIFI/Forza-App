/*
# Replace default chart template with full hierarchical chart of accounts

## What this does
Replaces the `seed_default_chart_template()` function with the complete,
detailed, hierarchical hotel chart of accounts the user provided — including
all parent (group) accounts and their sub-accounts linked via `parent_id`.

## Structure
The chart is organized in 5 main groups, each with sub-groups and detail accounts:
1. Assets (1000) — current (1100) and non-current (1200)
2. Liabilities (2000) — current (2100) and long-term (2200)
3. Equity (3000)
4. Revenue (4000) — rooms, F&B, services, other
5. Expenses & Costs (5000) — direct costs (5100), admin (5200), sales/marketing (5300), other (5400)

Each parent account is inserted first, then its children reference the parent
via `parent_id` using a temporary code→id map.

## Security
No table structure changes. RLS already enabled on chart_accounts and cost_centers.
The function remains SECURITY DEFINER so it can insert for the calling user.

## Notes
- Idempotent: ON CONFLICT (user_id, account_code) DO NOTHING.
- Safe to re-run.
- Cost centers seed unchanged.
*/
...
BEGIN
  ...
  -- Helper: insert a parent account and record its id by code
  -- We insert all accounts with explicit codes, then resolve parent links.
